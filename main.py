...
class AutoClickerApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.base_title = "自动点击工具 V1.3 - 稳定防卡死版"
    ...
    def on_coordinate_captured(self, x, y):
        rel_x, rel_y = x, y
        # 兼容后台：如果选择了目标窗口，需将屏幕绝对坐标转换为相对于窗口的相对坐标
    ...
    def update_steps_table(self):
        # 表格重绘核心：必须锁定信号 (blockSignals) 才能防止UI重绘引起的“单击变双击”等数据回流Bug
        ...
        for i, step in enumerate(self.steps):
            ...
            click_combo.blockSignals(True) # 渲染下拉框前必须加锁
            ...
            click_combo.blockSignals(False) # 渲染完成，解锁
...
class ExecutionThread(QThread):
    ...
    def find_image(self, image_path, similarity=0.8):
        ...
        # 读取图片模板 (使用 np.fromfile 防止中文路径 cv2 读取失败)
        try:
    ...
    def click_target(self, target_pos, step):
        ...
        if self.parent.target_hwnd:
            try:
                # 非常关键的坐标转换：必须将屏幕绝对坐标转换为目标窗口内部的客户区坐标，否则后台点击坐标必错！
    ...
    def click_target_backend(self, hwnd, x, y, down_msg, up_msg):
        ...
        # 发送"悬停(Hover)"信号给游戏引擎，预防引擎误认为凭空产生了点击事件
        ...
        # 延长"按压"停留时间，极大地防止因为游戏内部掉帧导致的"幽灵长按"卡死Bug
        time.sleep(random.uniform(0.06, 0.12)) 
        
        # 模拟真实抬起
        win32gui.PostMessage(hwnd, up_msg, 0, lparam)
        
        # 点击完成后释放坐标占用点，让按钮恢复原始未点击状态
        time.sleep(random.uniform(0.02, 0.05))
